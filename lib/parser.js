...
function getTok(){
  ...
  if(lastChar === ''){ //pretty obvious stuff here
    return {token:'EOF',index:input.getIndex()};
  }
  if(lastChar === '('){
  ...
  if(lastChar === '"'){   //Strings
    ...
    while(lastChar !== '"' || escaping){
      if(lastChar === ''){
        error.unexpectedToken({token:'EOF',index:input.getIndex()});
      }
  ...
  error.unexpectedToken({token:lastChar,index:input.getIndex()});
}
  ...
  if(section.token === 'number'){
    ...
    if(decimal !== -1){
      if(section.data.indexOf('.',decimal+1)!==-1){
        error.throw("Unnexpected second decimal place: " +
...
function parseExprs(tok){
  var t = tok || getTok(),tmp = [];
  while(t.token !== 'EOF'){
    tmp.push(parseExpr(t));
    t = getTok();
  }
  return tmp;
}
