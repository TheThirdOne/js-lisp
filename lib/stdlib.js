...
stdlib['|'] = {premap:true,codegen:function(arr){return code.writeLine('('+arr.join(' || ')+');');}};
stdlib['&'] = {premap:true,codegen:function(arr){return code.writeLine('('+arr.join(' && ')+');');}};
stdlib['!'] = {premap:true,codegen:function(arr){return code.writeLine('!'+stdlib['|'].codegen(arr) + ';');}};
...
stdlib.js   = {codegen:function(arr){
              for(var i in arr){
                if(!(arr[i].primary || arr[i].token === 'identifier')){error.throw('Unexpected non-primary',arr[i].index);}
              }
              return code.writeLine(arr[0].data + '.bind(' + arr[1].data + ');');
            }};
stdlib.import = {codegen:function(arr){
                if(arr[0].token !== 'string'){error.throw('Cannot dynamically compute imports',arr[0].index);}
                if(process.browser){
...
stdlib.let = {codegen:function(arr,codegen){
              var out = '';
              out += '(function(';
              if(arr[0].length === undefined){error.throw("Lacks argument list",arr[0].index);}
              var t = [[],[]], tmp = '';
              for(var i in arr[0]){
                if(arr[0][i].token !== 'identifier' && arr[0][i].length === undefined){
                  error.unexpectedToken(arr[0][i]);
                }
...
stdlib.def  = {codegen:function(arr,codegen){
                if(arr[0].token !== 'identifier'){error.unexpectedToken(arr[0]);}
...
stdlib.defun  = {codegen:function(arr,codegen){
                ...
                if(arr[0].length){
                  for(var i in arr[0]){
                    if(arr[0][i].token !== 'identifier'){error.unexpectedToken(arr[0][i]);}
                  }
